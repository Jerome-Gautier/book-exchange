        ...
        {#each users as user}
            <div class="bg-white rounded-lg shadow border border-gray-200 p-6 flex flex-col">
                ...
                <div class="flex flex-row gap-2 mt-4 md:mt-0">
                    {#if user.bookCount > 0}
                        <span class="bg-blue-100 text-blue-700 px-3 py-1 rounded text-sm font-medium">Books: {user.bookCount}</span>
                    {/if}
                    {#if user.incomingRequestsCount > 0}
                        <span class="bg-orange-100 text-orange-700 px-3 py-1 rounded text-sm font-medium">
                            Incoming requests: {user.incomingRequestsCount}
                        </span>
                    {/if}
                </div>
            </div>
